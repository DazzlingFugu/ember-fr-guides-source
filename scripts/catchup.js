...
if (shell.exec('git checkout -b catchup').code !== 0) {
  shell.echo('shelljs: "git checkout -b catchup" command failed');
  shell.exit(1);
}
...
if (shell.exec('git switch catchup').code !== 0) {
  shell.echo('shelljs: "git switch catchup" command failed');
  shell.exit(1);
}
    ...
    if (block.length) {
      ...
      fs.writeFile(`scripts/diff--${index}.diff`, `diff --git b/${block}`, function (err) {
        ...
        if (shell.exec(`git apply scripts/diff--${index}.diff`).code !== 0) {
          shell.echo(`shelljs: "git apply" command failed for diff--${index}.diff`);
        }
      });
    }
  });
